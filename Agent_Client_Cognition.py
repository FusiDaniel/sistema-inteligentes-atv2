...
# Esta parte do código inicializa o arquivo de configuração do NEAT e carrega o melhor agente treinado
...
# Este método consome o vector de sense e retorna o índice da saída do agente
def use_neat(vecInpSens: np.int32, onGoal, grabbed, onExit) -> int:
    ...
    return [0, 1, 3, 11, 12, 13][output.index(max(output))]

last_outy = -1
current_place = -1
onGoal, grabbed, onExit = False, False, False
def infer(vecInpSens: np.int32) -> int:
    global onGoal, grabbed, onExit, last_outy, current_place
    # print('infer: ', len(vecInpSens), ' ', vecInpSens)
    
    if last_outy == 0 and current_place == 4:
        grabbed = True
    onGoal = True if current_place == 4 else False
    onExit = True if current_place == 5 else False
    
    outy = use_neat(vecInpSens, onGoal, grabbed, onExit)
    if outy == 3:
        current_place = vecInpSens[0].tolist().index(1)
    last_outy = outy
    return outy
